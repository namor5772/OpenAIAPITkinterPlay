...
class ChatMemoryBot:
    def __init__(self, system_prompt: str, model: str = "gpt-4", max_tokens: int = 20000):
        self.client = OpenAI()
        self.model = model
        self.max_tokens = max_tokens
        try:
            # Works when tiktoken already maps your model name
            self.encoder = tiktoken.encoding_for_model(model)
        except KeyError:
            # Sensible fallback for models tiktoken doesn’t know yet
    ...
    def _summarize_history(self, old_messages: List[Dict[str, str]]) -> str:
        summary_prompt: List[ChatCompletionMessageParam] = [
            {"role": "system", "content": "Summarize the following chat history in at least 300 words"}
